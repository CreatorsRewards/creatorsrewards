...
import { Search, Plus, Edit2, Trash2, MoreVertical, Filter, Shield, Building, Users } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <header className="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        <div>
          ...
          <p className="text-gray-600">View, edit, and manage system access for all accounts.</p>
        </div>
              ...
              <tr key={user.id} className="hover:bg-gray-50 transition-colors group">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    {user.role === 'Creator' && <Users className="w-4 h-4 text-blue-500" />}
                    {user.role === 'Brand' && <Building className="w-4 h-4 text-purple-500" />}
                    {user.role === 'Admin' && <Shield className="w-4 h-4 text-gray-700" />}
                    <span>{user.role}</span>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                    ...
                    user.status === 'Pending KYC' ? 'bg-yellow-100 text-yellow-700' :
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded" title="Edit User">
                      <Edit2 className="w-4 h-4" />
                    </button>
                    <button className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded" title="Delete/Suspend">
                      <Trash2 className="w-4 h-4" />
                    </button>
                    <button className="p-1.5 text-gray-400 hover:bg-gray-100 rounded" title="More Options">
